...
?>
<dl class="mec-calendar-row simple">
        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                foreach($events[$today] as $event)
                {
                    ...
                    // print($event->data->meta['date']);
                    // echo $event->data->ID;
                    // print_r(get_field('site',$event->data->ID));
                    // echo get_field('site',$event->data->ID)->post_title;
                    ...
                    $site=get_field('site',$event->data->ID);
                    $trim_site_color = '#275371';

                    if($site){
                        $site_color = get_field('calendar_color',$site->ID);

                        $trim_site_color = substr($site_color, strpos($site_color, "#"));
                    }
                    ...
                    echo '<a class="mec-monthly-tooltip event-single-link-simple" event-id="'.$event->data->ID.'" data-tooltip-content="#mec-tooltip-'.$event_unique.'-'.$day_id.'" data-event-id="'.$event->data->ID.'" href="'.$this->main->get_event_date_permalink($event, $event->date['start']['date']).'"  '.$target_url.' style="background-color: '.$trim_site_color.';">';
