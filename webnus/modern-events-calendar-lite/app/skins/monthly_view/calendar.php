        ...
        for($x = 0; $x < $running_day; $x++)
        {
            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                foreach($events[$today] as $event)
                {
                    ...
                    if($this->display_detailed_time and $this->main->is_multipleday_occurrence($event)) $events_str .= '<div class="mec-event-detailed-time mec-event-time my mec-color"><i class="mec-sl-clock-o"></i> '.MEC_kses::element($this->display_detailed_time($event)).'</div>';
                    elseif(trim($start_time)) $events_str .= '<div class="mec-event-time my mec-color"><i class="mec-sl-clock-o"></i> '.esc_html($start_time.(trim($end_time) ? ' - '.$end_time : '')).'</div>';
        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                foreach($events[$today] as $event)
                {
                    ...
                    // elseif(trim($start_time)) $events_str .= '<div class="mec-event-time my2 mec-color"><i class="mec-sl-clock-o"></i> '.esc_html($start_time.(trim($end_time) ? ' - '.$end_time : '')).'</div>';
                        $events_str .= '<div class="mec-event-time my2 mec-color"> '.$output.'</div>';
            ...
            for($x = 1; $x <= (8 - $days_in_this_week); $x++)
            {
                ...
                if(isset($events[$today]) and count($events[$today]))
                {
                    ...
                    foreach($events[$today] as $event)
                    {
                        ...
                        if($this->display_detailed_time and $this->main->is_multipleday_occurrence($event)) $events_str .= '<div class="mec-event-detailed-time mec-event-time mec-color"><i class="mec-sl-clock-o"></i> '.MEC_kses::element($this->display_detailed_time($event)).'</div>';
                        elseif(trim($start_time)) $events_str .= '<div class="mec-event-time my3 mec-color"><i class="mec-sl-clock-o"></i> '.esc_html($start_time.(trim($end_time) ? ' - '.$end_time : '')).'</div>';
